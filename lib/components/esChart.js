...
class esChart extends Component {
  constructor(props, context) {
    ...
    this.handleZoomValueChange = this.handleZoomValueChange.bind(this);
  ...
  getQuarterFormat(dateValue) {
    ...
    let m = d.getMonth()+1;
    let retVal = '';
    if (m<=3) retVal = '1st Quarter of '+y;
    else if (m<=6) retVal = '2nd Quarter of '+y;
    else if (m<=9) retVal = '3rd Quarter of '+y;
    else retVal = '4th Quarter of '+y;
    return retVal;
  }
  ...
  getDateIndexes() {
    return this.props.dataApi.getDateIndexs();
  }

  getMarkets() {
    return this.state.showAllMarket
      ? this.props.dataApi.getMarkets()
      : this.props.dataApi.getMarkets().filter((i) => this.selectedMakets.includes(i.label));
  }
  ...
  handleZoomValueChange(value) {
    ...
    let newEnd = moment().endOf('year').toDate().getTime();
    let newStart;

    switch(value) {
      case '10y':
        newStart = moment(newEnd).add(-10, 'year').toDate().getTime();
        break;
      case '5y':
        newStart = moment(newEnd).add(-5, 'year').toDate().getTime();
        break;
      case '3y':
        newStart = moment(newEnd).add(-3, 'year').toDate().getTime();
        break;
      case '1y':
        newStart = moment(newEnd).add(-1, 'year').toDate().getTime();
        break;
      default:
        newEnd = this.getDataSeries().range().end().getTime();
        newStart = this.getDataSeries().range().begin().getTime();
    }

    const deltaStart = (newStart - currentStart) / 10;
    const deltaEnd = (newEnd - currentEnd) / 10;

    this.setState({
      selectedZoomYear: value
    });

    // One way to animate the change, should probably find a better way then this.
    let i = 0;
  ...
  render() {
    ...
    const range = this.state.timerange ? this.state.timerange : dataSeries.range();
        ...
        <div className='row' style={{ padding: '10px 0px 0px 0px' }}>
          ...
          <div className='col-md-6'>
            <div className='d-flex justify-content-end'>
              <a className='print-button' onClick={(e) => this.handlePrintClick(e)} />
            </div>
          </div>
        </div>
        <div className='row'>
          <div className='col-md-5'>
            ...
            <ToggleOptions list={[ '1y', '3y', '5y', '10y', 'All']} selection={this.state.selectedZoomYear} name={'zoomRange'} valueChange={this.handleZoomValueChange}/>
          </div>
          <div className='col-md-2'>
            <p className='d-flex justify-content-center'>Vacancy Rate</p>
          </div>
          <div className='col-md-5'>
          ...
          <div id='chart-container' className='col-md-12'>
            <Resizable>
              ...
              >
                ...
                <ChartRow height='500'>
                  <Charts>
                    <YearlyShader startTime={this.getStartDate()} endTime={this.getEndDate()} opacity={0.1}/>
                    <ScatterChart
                      axis='y'
                      series={dataSeries}
