...
class esChart extends Component {
  ...
  handleZoomRangeChange({ name, value }) {
    ...
    let startTime = currentRange.begin().getTime();
    let endTime = currentRange.end().getTime();

    if (name ==='forecastRange') {
      switch(value) {
        case '5y': {
          const fiveYear = moment().add(5, 'year').toDate().getTime();
          const maxTime = this.getDataSeries().range().end().getTime();
          endTime = fiveYear > maxTime ? maxTime : fiveYear;
          break;
        }
        case '3y':
          endTime = moment().add(3, 'year').toDate().getTime();
          break;
        case '1y':
          endTime = moment().add(1, 'year').toDate().getTime();
          break;
        default:
    ...
    if ( name === 'historicRange') {
      switch(value) {
        case '10y':
          startTime = moment().add(-10, 'year').toDate().getTime();
          break;
        case '5y':
          startTime = moment().add(-5, 'year').toDate().getTime();
          break;
        case '3y':
          startTime = moment().add(-3, 'year').toDate().getTime();
          break;
        case '1y':
          startTime = moment().add(-1, 'year').toDate().getTime();
          break;
        default:
          startTime = this.getDataSeries().range().begin().getTime();
      }
      this.setState({
  ...
  getForcastMarker() {
    return this.state.selectedForecastYears !== 'Off' 
      ? (
        <TimeMarker
          time={new Date()}
          label='Forecast'
