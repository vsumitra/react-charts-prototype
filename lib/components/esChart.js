...
class esChart extends Component {
  ...
  getColorIndex(column) {
    return this.colorList.indexOf(this.colorList.find((i) => i.assigned.includes(column)));
  }
  getStyle(column) {
    let colorIndex = this.colorList.findIndex((i) => i.assigned.length === 0);
    if (this.selectedMakets.includes(column)){
      colorIndex = this.colorList.indexOf(this.colorList.find((i) => i.assigned.includes(column)));
    }

    return {
      normal: {stroke: '#c0c0c0', fill: 'none', strokeWidth: 1},
      highlighted: {stroke: this.colorList[colorIndex].backgroundColor, fill: 'none', strokeWidth: 1},
      selected: {stroke: this.colorList[colorIndex].backgroundColor, fill: 'none', strokeWidth: 2},
  ...
  lineChartOnSelectionChange(selection) {
    ...
    if (this.selectedMakets.includes(selection)) {
      this.colorList[this.getColorIndex(market.label)].assigned = [];
      this.props.actions.removeMarketSelection(market.id);
    } else {
      this.colorList[this.colorList.findIndex((i) => i.assigned.length === 0)].assigned.push(market.label);
  ...
  render() {
    ...
    if(pointHover) {
      const color = this.colorList[this.getColorIndex(pointHover.column)];
      tooltipContent = (
        <div style={{opacity: .4, backgroundColor: color.backgroundColor, foreColor: color.foreColor}}>
                ...
                <ChartRow height='500'>
                  <Charts>
                    ...
                    <EventMarker
                      ...
                      markerLabel={infoValues}
                      infoTimeFormat='%Y'
                      infoWidth={120}
                      markerRadius={3}
                      markerStyle={{ fill: '#ec04fb' }}
                    />
                  </Charts>
                  <YAxis
                    id='y'
