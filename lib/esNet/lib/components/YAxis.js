...
export default class YAxis extends React.Component {
  constructor(props, context) {
    ...
    this.axisGenerator = this.getAxisGenerator();
  }
  ...
  updateAxis(scale, align, absolute, format, showGrid, chartWidth) {
    const labelStyle = merge(
      true,
      defaultStyle.labels,
      this.props.style.labelStyle ? this.props.style.labelStyle : {}
    );
    ...
    const { labelColor } = labelStyle;

    //
    // Make an axis generator
    //

    const axisGenerator =  this.axisGenerator(scale, align, absolute, format, showGrid, chartWidth);
  ...
  renderAxis() {
    ...
    this.axis = select(ReactDOM.findDOMNode(this)) // eslint-disable-line
      ...
      .style('font-weight', labelWeight || 100)
      .style(
        'font-size',
        labelSize ? `${style.width}px` : '12px'
      );
