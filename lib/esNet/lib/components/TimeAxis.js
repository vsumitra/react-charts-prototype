...
export default class TimeAxis extends React.Component {
  ...
  intervalToPixels(type, scale) {
    var refTick = moment(scale.domain()[0]).endOf(type).toDate();
    var nextTick = moment(refTick).add(1, type).endOf(type).toDate();
