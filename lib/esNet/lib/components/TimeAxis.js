...
export default class TimeAxis extends React.Component {
  ...
  renderTimeAxis(scale, format) {
    ...
    if (format === 'day') {
      ...
      axis = axisBottom(scale)
        ...
        .tickFormat((x) => {
          ...
          if ( mon <= 2 ) {
            return  'Q1 ' + yr.toString().slice(2,4);
          } else if ( mon <= 5 ) {
            return  'Q2 ' + yr.toString().slice(2,4);
          } else if ( mon <= 8 ) {
            return  'Q3 ' + yr.toString().slice(2,4);
          } else {
            return 'Q4 ' + yr.toString().slice(2,4);
          }
        })
        .tickSizeOuter(0);
    } else if (format === 'year') {
    ...
    if (moment(scale.domain()[1]).diff(moment(scale.domain()[0]), 'month', true) < 48 ) {
      ...
      select(ReactDOM.findDOMNode(this)) // eslint-disable-line
        ...
        .select('#minorTicks')
        .selectAll('.tick text')
        .attr('transform', 'translate(' + -this.intervalToPixels('month', scale, 3) / 2 + ',5)' );
    } else {

      if (scale.domain()[1].getMonth() < 9) {
        const textElements = select(ReactDOM.findDOMNode(this)) // eslint-disable-line
          .selectAll('#secondary text');
  
        const numberOfElements = textElements.size();
        select(textElements._groups[0][numberOfElements -1])
          .remove();
      }
  ...
  intervalToPixels(type, scale, interval = 1) {
    var refTick = moment(scale.domain()[0]).startOf(type).toDate();
    var nextTick = moment(refTick).add(interval, type).startOf(type).toDate();
