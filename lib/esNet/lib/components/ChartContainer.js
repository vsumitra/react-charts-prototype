...
export default class ChartContainer extends React.Component {
  ...
  render() {
    ...
    let yPosition = 0;
    this.props.children
      .filter((child) => child.type === ChartRow)
      .map((child) => {
        ...
        yPosition += parseInt(this.props.height, 10);
        i += 1;
      });
    ...
    this.props.children
      ...
      .map((child) => {
        const props = {
          timeScale: timeScale,
          height: chartsHeight - 15
        };
        const transform = `translate(${-leftWidth},0)`;
        markers.push(
          <g transform={transform} key={`chart-marker-${i}`}>
    ...
    const timeAxis = (
      <g transform={`translate(${leftWidth},${chartsHeight - 15})`}>
        <line x1={-leftWidth} y1={0.5} x2={timeAxisWidth} y2={0.5} style={xStyle} />
        <TimeAxis
          scale={timeScale}
          utc={this.props.utc}
...
ChartContainer.propTypes = {
  ...
  /**
   * The height of the chart. This library also includes a <Resizable> component
   * that can be wrapped around a \<ChartContainer\>. The purpose of this is to
   * inject a height prop into the ChartContainer so that it will fit the
   * surrounding element. This is very handy when you need the chart to resize
   * based on a responsive layout.
   */
