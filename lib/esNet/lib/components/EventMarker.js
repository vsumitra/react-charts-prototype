...
export default class EventMarker extends React.Component {
  ...
  renderMarker(event, column, info) {
    ...
    if (orgEvent.event instanceof TimeEvent) {
    ...
    let value;
    const { markerLabel } = this.props;
    if (this.props.yValueFunc) {
      value = this.props.yValueFunc(event, column);
    } else {
      value = event.get(column);
      if (!value && orgEvent.column){
        value =orgEvent.event.get(orgEvent.column);
      }
    }
    ...
    if (this.props.type === 'point') {
      ...
      if (tooltip._groups[0][0]) {
        tooltip
          .classed('show', true)
          .style('left', (posx - tooltip._groups[0][0].offsetWidth/2 + 6) + 'px')
          .style('top', (posy - tooltip._groups[0][0].offsetHeight - 10) + 'px');
      
        const arrow = tooltip.select('#chart-tooltip-arrow');
        arrow
          .style('left', (tooltip._groups[0][0].offsetWidth/2)  + 'px')
          .style('top', (tooltip._groups[0][0].offsetHeight - 2)  + 'px');

      } else if (_.isString(this.props.markerLabel)) {
        label = (
        ...
        label = (
          <g transform={`translate(${posx + dx - infoBoxProps.width/2},${posy + dy - (5 + infoBoxProps.height)})`}>
            <ValueList {...infoBoxProps} values={markerLabel} />
          </g>
        );
      }
  ...
  render() {
    const { event, column, info, pointEvent } = this.props;
    if (!event && !pointEvent) {
      this.hideTooltip();
      return <g />;
    }
    if (!event) {
      return (
        <g>
          {this.renderMarker(pointEvent, column, info)}
        </g>
      );
    }
    return (
...
EventMarker.propTypes = {
  ...
  /**
   * Alternate event from scatter plots.
   */
  pointEvent: PropTypes.shape({
    event: PropTypes.oneOfType([
      PropTypes.instanceOf(TimeEvent),
      PropTypes.instanceOf(IndexedEvent),
      PropTypes.instanceOf(TimeRangeEvent)
